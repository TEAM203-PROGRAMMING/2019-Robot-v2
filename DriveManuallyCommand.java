...
public class DriveManuallyCommand extends Command {
 ...
 requires(Robot.driveSubsystem); //this is command is commanding the robot subsystem
  ...
  protected void execute() { //gets called everything command loop fires, rus over and over again
    ...
    double move = Robot.oi.stick.getRawAxis(1); //move variable controls forward and backwards movement (Y axis is up and down!)
    double turn = -Robot.oi.stick.getRawAxis(4); //move variable controls left and right movement (X xis is left and right!)
    Robot.driveSubsystem.arcadeDrive(move, turn);
